...
import static com.afonsoraposo.camera.CameraUtils.computeBestPreviewSize;
...
public class Camera {
  ...
  public Camera(
    ...
    this.whiteBalance = parseWhiteBalance(whiteBalance);
  ...
  private int parseWhiteBalance(String whiteBalance){
      switch(whiteBalance){
            case "off":
              return CameraMetadata.CONTROL_AWB_MODE_OFF;
            case "incandescent":
            ...
            case "twilight":
              return CameraMetadata.CONTROL_AWB_MODE_TWILIGHT;
            case "shade":
  ...
  private void createCaptureSession(int templateType, Surface... surfaces)
    ...
    private void disableAutomatics(CaptureRequest.Builder captureRequestBuilder) throws CameraAccessException {
            ...
            try {
              ...
              cameraCaptureSession = session;
                captureRequestBuilder.set(
                        CaptureRequest.CONTROL_AWB_MODE, whiteBalance); // disable auto-white-balance
                if(iso!=0 || shutterSpeed!=0L){
                    captureRequestBuilder.set(
                      CaptureRequest.CONTROL_AE_MODE, CameraMetadata.CONTROL_AE_MODE_OFF); // disable auto-expose
                }

                if(focusDistance!=0){
                    captureRequestBuilder.set(
                            CaptureRequest.CONTROL_AF_MODE, CameraMetadata.CONTROL_AF_MODE_OFF); // disable auto-focus
                    captureRequestBuilder.set(CaptureRequest.LENS_FOCUS_DISTANCE, focusDistance); // lock focus
                }
                if(shutterSpeed!=0L){
                    captureRequestBuilder.set(CaptureRequest.SENSOR_EXPOSURE_TIME, exposureTime); // exposition time
                    captureRequestBuilder.set(CaptureRequest.SENSOR_FRAME_DURATION, shutterSpeed); // shutter speed 1/30
                }
                if(iso!=0){
